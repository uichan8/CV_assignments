"""
# Kookmin University SoEE Computer Vision Assignment 4
By Soochahn Lee
modified from CSCI 1430 Deep Learning project By Ruizhao Zhu, Aaron Gokaslan, James Tompkin
...
Usage: 
    python main.py -data [DATA] -net [NETWORK] -mode [MODE]
        DATA    | "mnist" or "scenerec"
        NETWORK | "one_layer" or "cnn"
        MODE    | "torch" or "nn" or "svm" (only for one_layer network)
"""
...
from model_20172099 import Model
import os
import sys
import argparse
from model_pytorch_20172099 import train_pytorch, test_pytorch
...
parser.add_argument("-net", help="Designates network model type. Valid values: one_layer, cnn")
parser.add_argument("-mode", help="Designates classifier to use. Valid values: torch, nn, nn+svm")
parser.parse_args()
...
def main(argv):
    ...
    data = {"mnist", "scenerec"}
    net = {"one_layer", "cnn"}
    mode = {"torch", "nn", "nn+svm"}

    if argv[1] not in data:
        raise ValueError("Data must be one of %r.", data)

    if argv[3] not in net:
        raise ValueError("Mode must be one of %r.", net)

    if argv[5] not in mode:
    ...
    if argv[3] == "one_layer":
        ...
        model = train_pytorch(train_images, train_labels, num_classes)
        accuracy = test_pytorch(model, test_images, test_labels)
        print ('pytorch cnn model training accuracy: {:.0%}'.format(accuracy))
